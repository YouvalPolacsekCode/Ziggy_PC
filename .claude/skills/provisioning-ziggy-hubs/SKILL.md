...
| Rebuilt ziggy but the **mobile app never pulls the update** | a manual `--build` doesn't set the `GIT_SHA` build-arg, so the image ENV `ZIGGY_GIT_SHA` stays `dev`; the app's OTA check (`/api/mobile/version`) sees no version change and never downloads the new web bundle | rebuild passing the SHA: `cd /opt/ziggy && SHA=$(sudo git rev-parse --short HEAD) && sudo env GIT_SHA=$SHA docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build ziggy`. Verify `docker exec ziggy-ziggy-1 printenv ZIGGY_GIT_SHA` = the SHA and `curl -s -o /dev/null -w '%{http_code}' localhost:8001/api/mobile/bundles/<sha>.zip` = 200. Frontend (React) fixes reach the phone via this OTA bundle — **no APK/ADB reinstall** unless native code (capacitor.config, plugins, permissions, manifest) changed. App applies it over two launches (download, then swap). |
