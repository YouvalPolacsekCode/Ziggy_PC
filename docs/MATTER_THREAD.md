...
| Device found + connects over BLE but fails: `BluezEndpoint Internal error` / `Operation was cancelled` / `le-connection-abort-by-local` | BlueZ not in **experimental** mode — Matter/CHIPoBLE needs BLE GATT features Ubuntu doesn't enable by default | systemd drop-in: `ExecStart=…/bluetoothd --experimental` (the enable script sets this). |
| BLE connects + attests, then `Required network information not provided / thread (no)` | matter-server has no Thread dataset; HA's `matter/commission` does NOT auto-attach it | Ziggy calls `matter/set_thread` with HA's preferred dataset TLV *before* `matter/commission` (`services/ha_pairing.py`). |
| Pairing worked once, now every attempt fails at BLE connect | Adapter in a **degraded state** after many rapid/overlapping attempts (retry storm); the frontend's early "request timed out, try again" invites re-clicks that abort each other's BLE connection | Reset the adapter (`hciconfig hci0 down/up`) + restart matter-server for a clean slate, then **one** attempt with the device freshly reset + close to the hub. Confirm the device advertises the Matter service with an independent scan: `btmon \| grep -i fff6`. TODO: make commissioning async (fire+poll) so the UI never times out into a retry storm. |
