...
- Every service below also has a founder-language entry in the Desk's knowledge
  registry (purpose, why, where the data comes from, what runs automatically, what you
  do, when to open the vendor, plan, limit, cost, fallback) — shown on `/system`
  (Services) and answerable via `/ask`. `KNOWLEDGE.md` is generated from it.
...
### n8n — removed (see `N8N_DECISION.md`)

- Audited against the Desk's own outbox, scheduler, connectors and inbox: every planned
  workflow is native now (daily digest `desk/company/digest.py`; support mail
  `POST /api/company/support/inbound/{token}`; fleet-down → urgent inbox item → webhook).
  No always-on machine, no second system. The `n8n/` folder lives in git history
  (commit `78ea112`) if it is ever needed.
