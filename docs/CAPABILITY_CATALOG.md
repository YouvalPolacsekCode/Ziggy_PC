...
- **Status:** `canary-only` — services/light_hold.py hooked from services/ha_subscriber.py and the scheduler tick; enforced in local_automation_actions' call_service step for respect_hold turn-ons, which ha_automations compiles to a deferred event so HA hands them to Ziggy; tests/test_light_hold*.py, tests/test_executor_hold_gate.py, tests/test_why_not_hold.py.
...
- **Surfaces:** `services/light_hold.py`, `services/ha_subscriber.py`, `services/local_automation_actions.py`, `backend/routers/light_hold_router.py`, `frontend/src/components/device/DeviceCard.jsx`, `services/why_not.py`
- **Known gaps:** Only rules built by Ziggy's recipes and by the chat designer carry the hold; an automation hand-built in the wizard with a plain turn_on ignores it until its turn_on is marked respect_hold.; Lights only — a fan or a plug switched off by hand is still governed by the older 30-minute manual-override window.; Hold-aware turn-ons run through Ziggy rather than natively in Home Assistant, one extra hop; measured on the Canary as part of acceptance.
