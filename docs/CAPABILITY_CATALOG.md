...
**79 capabilities** built from **58 shared mechanisms**.
...
Five fixed modes a household can flip from the Home screen, from chat ("movie mode for two hours"), from a wireless button, or from an automation step. Each has one defined effect the engine enforces: Sleep and Movie stop motion from turning lights on (Sleep ends at the morning time, Movie after three hours); Cleaning stops "off when empty" rules for two hours; Guests stops "everyone left" from running while people without tracked phones are in the house; Vacation switches on the lived-in evening simulation. Automations can condition on a mode, and the same mode is visible to Home Assistant-compiled rules through a mirrored sensor, so the two evaluators never disagree. There are no other modes: the flags the chat designer used to invent (and nothing ever read) are gone.

- **Status:** `canary-only` — services/modes.py, services/modes_mqtt.py, backend/routers/modes_router.py, frontend/src/components/home/ModeChips.jsx; tests/test_modes*.py. Built 2026-09-18 on worktree-designer-closed-loop after the operator's living-area session showed two designer-invented mode flags with zero readers.
- **Layer:** Automations · **Audience:** user-facing
- **Shipped:** 2026-09-18
...
- **Surfaces:** `services/modes.py`, `services/modes_mqtt.py`, `backend/routers/modes_router.py`, `frontend/src/components/home/ModeChips.jsx`, `frontend/src/pages/Dashboard.jsx`, `core/agent/tools.py`
- **Known gaps:** Modes are home-wide; a per-room Sleep (one bedroom asleep, the rest of the house awake) is not a thing yet — Smart Room's occupancy edge still covers the sleeping-person case on its own.; Vacation starts the lived-in simulation only when at least one room has a dimmable light; otherwise it just holds the flag.
...
- **Known gaps:** Inbound webhooks stay declined on purpose until a security review; calendar, NFC-tag and template triggers remain genuine gaps.; Blueprint bodies (wait_for_trigger, choose, repeat) are still the only route to branching inside one rule; the designer builds one rule per case instead.
...
- **Composes with:** `assistant-agent-v2` (via assistant-engine-flag, intent-dispatcher), `buttons-from-chat` (via intent-dispatcher), `cloud-brain-relay` (via llm-relay-chat-client), `design-from-chat` (via intent-dispatcher), `household-organizer` (via intent-dispatcher), `live-answers` (via intent-dispatcher) …and 1 more
...
- **Composes with:** `buttons-from-chat` (via ha-truth-directory, intent-dispatcher), `chat-with-ziggy` (via assistant-engine-flag, intent-dispatcher), `design-from-chat` (via ha-truth-directory, intent-dispatcher), `household-organizer` (via intent-dispatcher), `live-answers` (via intent-dispatcher)
...
When a person turns a light off — wall switch, the app, chat or voice; anything that isn't one of Ziggy's own engines — Ziggy holds it off so Smart Room and Motion Light leave it alone. Turning it on yourself clears the hold. The room being empty for thirty minutes releases it. If a rule then relights it and you switch it off again, Ziggy concludes "off tonight" and holds it until the morning time. The light's tile shows "Held" with a Release action, and asking "why didn't the kitchen light come on?" gets the honest answer: you turned it off yourself.

- **Status:** `canary-only` — services/light_hold.py hooked from services/ha_subscriber.py and the scheduler tick; enforced in local_automation_actions' call_service step for respect_hold turn-ons, which ha_automations compiles to a deferred event so HA hands them to Ziggy. Field-corrected 2026-09-19 after the operator's office lamp was relit 8s after a manual off: an HA-executed automation set no attribution and read as the person, erasing the hold (light_hold.note_automation_fired) and the installed rules predated respect_hold (services/hold_migration.py). tests/test_light_hold*.py, test_light_hold_attribution.py, test_hold_migration.py, test_executor_hold_gate.py.
- **Layer:** Climate & light · **Audience:** user-facing
- **Shipped:** 2026-09-18
- **Built from:** `bundle-executor`
- **Composes with:** `actions-page-and-library` (via bundle-executor), `buttons-from-chat` (via bundle-executor), `design-from-chat` (via bundle-executor), `home-modes` (via bundle-executor), `leaving-and-arriving` (via bundle-executor), `more-ready-made-routines` (via bundle-executor) …and 1 more
...
- **Known gaps:** A rule the customer built themselves in the wizard still switches a held light on — only Ziggy's own recipes ask about the hold. It no longer erases the hold when it does, so the hold survives until the room empties.; Recipe automations installed before 2026-09-19 are upgraded in place by a background migration rather than by the customer re-saving them; until that pass runs, they relight a held light.; Lights only — a fan or a plug switched off by hand is still governed by the older 30-minute manual-override window.; Hold-aware turn-ons run through Ziggy rather than natively in Home Assistant, one extra hop; measured on the Canary as part of acceptance.
...
- **Used by:** `assistant-agent-v2`, `buttons-from-chat`, `design-from-chat`
